...
def check_params(previous_password, new_password, repeat_new_password, user_mail):
    ...
    for s in [previous_password, new_password, repeat_new_password, user_mail]:
        if " " in s:
            return StateConstants.pwd_contain_space() | {"flag": False}

    # 检查密码长度是否大于限制
    for s in [previous_password, new_password, repeat_new_password]:
        if len(s) > public_platform.pwd_len_upper_limit:
            return StateConstants.password_out_limit() | {"flag": False}

    # 检查字符串长度是否小于4
    for s in [previous_password, new_password, repeat_new_password]:
        if len(s) < public_platform.pwd_len_lower_limit:
            return StateConstants.password_less_length() | {"flag": False}

    # 检查两次新密码是否不一致
    if new_password != repeat_new_password:
        return StateConstants.pwd_inconsistent() | {"flag": False}

    # 检查邮箱正则
    if not re.match(public_platform.mail_regex_rule, str(user_mail)):
        return StateConstants.invalid_email() | {"flag": False}

    # 检查参数是否为空
    for s in [previous_password, new_password, repeat_new_password]:
        if not s:
            return StateConstants.param_empty() | {"flag": False}
...
def contrast_prev_pwd(user_email, commit_old_pwd):
    ...
    if user_tuple == None:
        return StateConstants.user_status_amiss() | {"flag": False}
    ...
    if prev_password != commit_old_pwd:
        return StateConstants.origin_password_incorrect() | {"flag": False}

    return {"salt": salt_key, "flag": True}
